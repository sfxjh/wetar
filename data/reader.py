...
class BaseReader(object):
    ...
    def _convert_instance_to_dataset(self,
        ...
        for instance in iterator:
            ...
            input_num_id = int(instance.idx.split('-')[-1])

            all_ids.append((input_ids, attention_mask, token_type_ids, label, input_len, input_num_id))
        ...
        all_input_num_id = torch.tensor([f[5] for f in all_ids], dtype=torch.long)

        none_labels = any([True if f[3] is None else False for f in all_ids])
        if none_labels:
            dataset = TensorDataset(all_input_ids, all_attention_mask, all_token_type_ids, all_lens, all_input_num_id)
        else:
            all_labels = torch.tensor([f[3] for f in all_ids], dtype=torch.long)
            dataset = TensorDataset(all_input_ids, all_attention_mask, all_token_type_ids, all_lens, all_labels, all_input_num_id)
...
class ClassificationReader(BaseReader):
    ...
    def read_from_file(self, file_path, split='train'):
        temp_label_set = set()
        instances = []
        if os.path.exists(os.path.join(file_path, f'{split}.csv')) or (file_path.split('.')[-1] == 'csv' and split == None):
            if split != None:
                file_path = f'{file_path}/{split}.csv'
            with open(file_path, 'r', encoding='utf-8') as fin:
                count = 0
                reader = csv.reader(fin)
                for row in reader:
                    if len(row[0]) == 0:
                        if len(row) == 3 or (len(row) == 4 and row[-1] == 'sentence' and row[-2] == 'label'):
                            sentence_num = 1
                        else:
                            sentence_num = 2
                        continue
                    if sentence_num == 1:
                        temp_label_set.add(row[-2])
                        instances.append(InputInstance(row[-3], row[-1], None, row[-2]))
                    elif sentence_num == 2:
                        if row[-1] == '-1':
                            continue
                        temp_label_set.add(row[-1])
                        instances.append(InputInstance(row[0], row[1], row[2], row[-1]))
                    count += 1
        else:           
            if split != None:
    ...
    def get_dataset_loader(self, dataset, tokenized=True,
                           batch_size=32, shuffle=False, distributed=False):
        ...
        if not distributed:
            ...
            data_loader = DataLoader(dataset, batch_size=batch_size, shuffle=shuffle, collate_fn=collate_function, sampler=data_sampler)
        return data_loader
